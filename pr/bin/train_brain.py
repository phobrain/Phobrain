...
## === train_brain.py - train phobrain nets at different stages
##
##          search MAIN below for main run params

## == misc notes

# pair_dir = JAVA2ML_BASE + '/pairs/2023_01/h' # PlanG
# pair_dir = JAVA2ML_BASE + '/pairs/2023_04/v' # PlanG
# 2023_01 vb/JH2 got to 90_90, softsign (2x2080ti, @72,91min)

# for /usr/bin/ptxas (? tf expects it ?), and various libs come with:
...
##### temp while migrating to cmdline args
LEGACY = False

# GCTF: https://github.com/Rishit-dagli/Gradient-Centralization-TensorFlow
...
print('-- IMAGE_DESC_DIR is ' + IMAGE_DESC_DIR + ' Training data in ' + JAVA2ML_BASE)
print('-- HOST ' + HOST)
...
def usage(msg):
    ...
    print('\nUsage: ' + sys.argv[0] + '\n' + \
                    ...
                    '\t -<v|h>\n' + \
                    '\t -pair_dir <p>\n' + \
                    '\t -in_mode <JPGV_HIST|VEC|...>\n' + \
                    '\t     JPGV_HIST: -jpg <vectors> -side_vec_len <len>\n' + \
                    '\t -optimizer <o1,o2,o3..>\n' + \
                    '\t     oX in [SGD,STEEP,RMS,Adam]\n' + \
                    '\t -activation <a1,a2,a3..>\n' + \
                    '\t     aX in [relu,tanh,swish,softsign]\n' + \
                    '\t -run <cycle|>')
    exit(1)
...
while ix < len(args):
    ...
    elif arg == '-side_vec_len':

        ix += 1
        arg = args[ix]

        SIDE_VEC_LEN = int(arg)
...
if pair_dir is None:
    usage('Need -pair_dir <dir>')

pair_dir = os.path.expanduser(pair_dir)

if IN_MODE is None:
    usage('Need -in_mode <xxx>')

if IN_MODE not in ['JPGV_HIST', 'VEC']:
    print('==== UNUSUAL: -in_mode: ' + IN_MODE)

if 'JPGV' in IN_MODE:
    DO_JPGV = True

if IN_MODE == 'JPGV_HIST':

    err = False
    if SIDE_VEC_LEN is None:
        err = True
    if CMD_JPG_DIR is None:
        err = True
    if err:
        usage('JPGV_HIST needs -side_vec_len and -jpg (vec_dir)')

    jpg_dir = CMD_JPG_DIR

ACTIVATION = 'relu'

if CMD_ACTIVATION is None:
    pout('Using default activation: ' + ACTIVATION)
else:
    ACTIVATION = CMD_ACTIVATION

if CMD_OPTIMIZERS is None:
    pout('Using default optimizer: SGD')
    CYCLE_LIST = [ 'SGD' ]
elif CMD_OPTIMIZERS == 'SSR':
    CYCLE_LIST = [ 'SGD', 'STEEP_SGD', 'RMSProp']
elif CMD_OPTIMIZERS == 'AR':
    CYCLE_LIST = [ 'Adam', 'RMSProp' ]
elif CMD_OPTIMIZERS == 'RA':
    CYCLE_LIST = [ 'RMSProp', 'Adam' ]
elif CMD_OPTIMIZERS == 'SSRA':
    CYCLE_LIST = [ 'SGD', 'STEEP_SGD', 'RMSProp', 'Adam']
else:
    # could be just one
    CYCLE_LIST = [ CMD_OPTIMIZERS ]

if LOAD_THREADS < 1:
    usage('-threads <1: ' + str(LOAD_THREADS))
...
if RUN_CPU:
    ...
    print('-- RUN_CPU  devs=' + str(len(physical_devices)))
    ...
    print('-- RUN_GPU  devs=' + str(len(physical_devices)))
    ...
    os.environ['CUDA_VISIBLE_DEVICES'] = str(list(range(len(physical_devices))))
    ...
    if DISTRIBUTE:
        if 'CUDA_VISIBLE_DEVICES' in os.environ:
            device_env = os.environ['CUDA_VISIBLE_DEVICES']
        else:
            device_env = None

        print('-- DISTRIBUTE: ' + str(DISTRIBUTE) + \
                ' env CUDA_VISIBLE_DEVICES: ' + str(device_env))

if DISTRIBUTE is None:

    usage('internal: DISTRIBUTE not set')
...
# basic yes/no 0..1 sort:
LOSS_FUNCTION = 'binary_crossentropy'
FINAL_ACTIVATION = 'sigmoid'

print('-- loss: ' + LOSS_FUNCTION + '  final_activation: ' + FINAL_ACTIVATION)
...
#  v/JH3/VGG16 - 2022_06 - redid training data; lrv=2
#                   relu - marginal=loss.stuck(?)
#                   softsign - ok
#                   swish - ok
#  vb/JH3/VGG16 - 2022_04
#                   softsign->marginal (most negs in 70's)
#  vb/JH2/VGG16 - 2022_05
#                   Adam no good w/ relu
#  vb/JH2/VGG16 - 2023_01
#                   90_90 w/ softsign
...
#
# Now set by args | ACTIVATION = 'relu'

# softsign
#   not good h/JH/glorot_normal(he_normal for v)
#   good on hb/JH,
#   skip on v/JH12, 2023_01
#   vb/JH2, 2023_01: 90_90 SGD (Adam, RMSProp no good)
# Now set by args | ACTIVATION = 'softsign'

# tanh no good on JH
# Now set by args | ACTIVATION = 'tanh'

# swish
#   vb/JH2 2023_01: 92_87, 91_89
#   ok on v/JH12
#   WILL be switched to relu for IN_MODE == 'VEC'
# Now set by args | ACTIVATION = 'swish'

######## more-atomic IN_MODEs used in past,
# only for further training of imagenet models,
# normally use V=feature vectors, rather than process jpgs
#IN_MODE = 'JPG_HIST'
...
if 'HIST' in IN_MODE:
    ...
    # Prerolling IN_HIST_MODE = 'GREY128_SAT128_RGB12'
    #   for single load:
    IN_HIST_MODE = 'MLHIST'
...
if 'JPG' in IN_MODE:
    ...
    while jpg_dir.endswith('/'):
        jpg_dir = jpg_dir[:-1]

    if jpg_dir.endswith('MobileNetV2'):
        IN_JPG_MODE = 'MobileNetV2_224';
    elif jpg_dir.endswith('DenseNet121'):
        IN_JPG_MODE = 'DenseNet121_224';
    elif not jpg_dir.endswith('VGG16'):
        print('== Unexpected type, may not matter for vecs: ' + jpg_dir)
        print('   Using ' + IN_JPG_MODE)
...
# Now set by args | typically SIDE_VEC_LEN = 2
...
if TUNER:
    #CYCLE_LIST = [ 'RMSProp' ]
    CYCLE_LIST = [ 'Adam' ]
    print('TUNER: ' + CYCLE_LIST)

print('--  CYCLE_LIST: ' + str(CYCLE_LIST))

# replaced by opt_str from cmd line | manual: always try SGD first
...
if not DO_VEC and SIDE_VEC_LEN < 3:
    # Adam on vb JH: 69:72, 72:70, 74:68 / 2022_05 / zilch JH2/relu 2023_01
    # RMSProp vb JH: 86:85 after 100 mins / zilch JH2/relu 2023_01
    # 2023_01: same for JH2 + softsign
    print('-- OVERRIDE CYCLE_LIST: only SGD optimization for SIDE_VEC_LEN < 3')
...
if DO_VEC:
    ...
    #vec_list_file = 'h_2022_10/all_vecs'
    vec_list_file = 'v_2023_03/jh_all_vecs'
...
if DO_HIST:
    ...
    if 'MLHIST' in IN_HIST_MODE:

        hist_dirs.append(IMAGE_DESC_DIR + '/2_hist/pics/ml_concat')
        hist_load_dims.append(1984)
        hist_dense_sizes.append(1024) # per squash of rgb12 1728->728
        hist_alg += 'ml1984_'
...
def procId(fname):
    ...
    tag = tag.replace('DSC', '')
    # TODO:load custom tags from phobrain_local/
...
def load_vecs():
    ...
    print('Loading phobrain side vecs; getting dims from file names:')
    ...
    for i in range(len(side_vec_files)):
        f = side_vec_files[i]
        dim =  side_vec_dims[i]
        print('Loading side vecs: ' + f + ' dim ' + str(dim))
        vec_len = 0
        with open(f) as fp:
    ...
    print('Loaded side vecs: ' + str(len(side_vec_files)) + ' tot_dim ' + \
...
def load_dbvecs():
    ...
    print('Load dbvecs, threads: ' + str(LOAD_THREADS))
    pool = Pool(processes=LOAD_THREADS)
...
def load_kwdvecs():
    ...
    print('Load kwdvecs, threads: ' + str(LOAD_THREADS))
    pool = Pool(processes=LOAD_THREADS)
...
def load_it_hist(arg):
    ...
    if hist_fname.endswith('.hist_bin'):
        with open(hist_fname, 'rb') as f:
            x = np.fromfile(f, dtype='>f', count=-1, sep='')
    else:
    ...
    for hist_model in range(1, len(hist_dirs)):
        ...
        if hist_fname.endswith('.hist_bin'):
            with open(hist_fname, 'rb') as f:
                x1 = np.fromfile(f, dtype='>f', count=-1, sep='')
        else:
    ...
    if np.isnan(sum(x)):
        ...
        print('load_it_hist(' + hist_fname + '): sum(x) is nan. len(x) is ' + str(len(x)))
        print('  len(hist_dirs): ' + str(len(hist_dirs)))
        print('Exiting')
        exit(1)

    #print('Final Sum ' + str(sum(x)) + ' Max ' + str(max(x)))
...
def load_preproc():
    ...
    if DO_JPGV or DO_JPG:
        ...
        if DO_JPG:
            print('Load jpgs, threads: ' + str(LOAD_THREADS))
        else:
            print('Load jpg vecs, threads: ' + str(LOAD_THREADS))
    ...
    if DO_HIST:
        ...
        pool = Pool(processes=LOAD_THREADS)
        print('Load hists, threads: ' + str(LOAD_THREADS))
        l = []
...
def jpg_name(fname):
    ...
    if not os.path.isfile(fname):
        print('jpg_name: No file like: ' + fname)
        quit()
...
def hist_name(model, fname):
    ...
    fname_bin = fname + '_bin'

    actual_fname = None

    if os.path.isfile(fname_bin):
        actual_fname = fname_bin
    ...
    fname = os.path.expanduser(actual_fname)

    if not os.path.isfile(fname):
        print('hist_name: No file like: ' + fname)
        quit()
...
def create_siamese_net():
    ...
    if DO_HIST:
        ...
        if 'ml' in hist_alg:
            ...
            alg_notes += hist_alg + str(hist_input_dim) + '_'
    ...
    if DO_JPGV:
        ...
        '''
        if 'VGG16' in IN_JPG_MODE:
            jpg_vec_dir = jpg_dir + '/VGG16'
        elif 'VGG19' in IN_JPG_MODE:
            jpg_vec_dir = jpg_dir + '/VGG19'
        elif 'DenseNet121' in IN_JPG_MODE:
            jpg_vec_dir = jpg_dir + '/DenseNet121'
        elif 'DenseNet169' in IN_JPG_MODE:
            jpg_vec_dir = jpg_dir + '/DenseNet169'
        elif 'ResNet152V2' in IN_JPG_MODE:
            jpg_vec_dir = jpg_dir + '/ResNet152V2'
        elif 'Xception' in IN_JPG_MODE:
            jpg_vec_dir = jpg_dir + '/Xception'
        elif 'NASNetLarge' in IN_JPG_MODE:
            jpg_vec_dir = jpg_dir + '/NASNetLarge'
        elif 'EfficientNetB7' in IN_JPG_MODE:
            jpg_vec_dir = jpg_dir + '/EfficientNetB7'
        '''
...
def stem_sigmoid(left, right, side_vec_len, hp):
    ...
    sigmoid = Dense(1, activation=FINAL_ACTIVATION)(merged)
    return sigmoid

START_EPOCHS = None

def optimizer_extras(opt_str):
    ...
    if (DO_JPGV or DO_JPG) and \
            opt_str == 'adadelta' and \
            batches[BATCH_INDEX_START] > 96:
        print('Recommend for jpg+adadelta drop batch ' + \
                        str(batches[BATCH_INDEX_START]) + '->96')
        #batch_size = 96

    print('-- imports for OPT=' + str(opt_str));
...
def create_network(in1, in2, hp, opt_str):
    ...
    if GCTF:
        ...
        if 'SGD' in opt_str:
            ...
            if not DO_VEC and SIDE_VEC_LEN < 3:
                # won't train for vb+he_normal w/ .01, .001
                print('-- Adam lrv<3: gctf.optimizers.adam: ' + \
                         ' vb: tried down to 1e-10/nada, using 1e-5')
                opt = gctf.optimizers.adam(learning_rate = 1e-5)
            else:
                opt = gctf.optimizers.adam(learning_rate = 0.01)
                print('-- Adam: gctf.optimizers.adam')
        else:
            print('-- todo.gtcf: ' + opt_str)
            quit()
...
def new_model(hp, opt_str):
    ...
    model = create_network(in1, in2, hp, opt_str)
    ...
    if ' ' in str(opt_str):
        notes = alg_notes + '_' + type(opt_str).__name__
    else:
        notes = alg_notes + '_' + str(opt_str)
    ...
    if DO_VEC and side_vec_dim < 16:
        if ACTIVATION == 'tanh':
            ...
            # more epochs used to be needed
            #epochs = 30
            print('-- side_vec_dim < 16: starting epochs=' + str(epochs) + ' and random_batch')
            REFINE_STRATEGY = 'random_batch'
...
def cycle_optimizers():
    ...
    print('--- cycle_optimizers: ' + str(CYCLE_LIST))

    for opt_str in CYCLE_LIST:

        print('------------ cycle: ' + opt_str + '   =======>>>')

        optimizer_extras(opt_str)

        if opt_str == 'STEEP_SGD' and modname != None:
        ...
        for i in range(9):
            doit(modname == None, opt_str)

#distrib_strategy = None

def doit(new, opt_str):

    global distrib_strategy
...
def orig_doit(new, opt_str):
    ...
    if round_ct > 1:
        ...
        if skipped_rounds > 2:
            ...
            if 'VEC' in IN_MODE:
                print('-- TODO: handle VEC w/ skipped_rounds limit')
                exit(1)
            ...
            if ACTIVATION == 'relu':
                ...
                print('-- No start w/ basic JvH activations')
                exit(1)

            print('-- SKIPPED ' + str(skipped_rounds) + ' ROUNDS:' + \
                        ' updating activation from ' + ACTIVATION + \
                        ' to ' + new)
            ACTIVATION = new
            skipped_rounds = 0
    ...
    if opt_str == 'SWA_SGD':
        ...
        if LOSS_HISTORY:
            ...
            if 'JPG' in IN_MODE and SIDE_VEC_LEN == 2 and epochs == 3:
                # detect loss==0.6932?
                print('-- jpg/lrv2: adding loss=0.6932,patience=1 early stop for epochs=' + str(epochs))
                callbacks.append(tf.keras.callbacks.EarlyStopping(monitor='loss', baseline=0.6932, \
                                                                                min_delta=0.0001, \
                                                                                patience=1, verbose=1))

            if epochs > 5:
                print('-- adding val_loss,patience=2 early stop for epochs=' + str(epochs))
                callbacks.append(tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=2, verbose=1))
                #callbacks.append(tf.keras.callbacks.EarlyStopping(monitor='loss', patience=2))
            ...
            last_loss = new_history.history['loss'][-1]
            ...
            if prev_history != None:
                ...
                if new_history.history['loss'] > prev_history.history['loss']:
                    print('--- LOSS INCREASE: ' + str(prev_history.history['loss']) + ' -> ' +\
                                                str(new_history.history['loss']))
    ...
    if last_loss > 0.6928:
        print('--- BREAK for stuck loss on 1st round')
        del model
        modname = None
        return
    ...
    tr_optimizer_list.append(opt_str)
    ...
    orient_train = orient
    if pair_dir.endswith('b') or pair_dir.endswith('b/'):
        orient_train += 'b'

    loss = new_history.history['loss'][-1]
    loss = int(loss * 10000)
    val_loss = new_history.history['val_loss'][-1]
    val_loss = int(val_loss * 10000)

    modname = 'm_' + orient_train + '_model_pnlv_' + \
                        str(int(100*te_pos_acc)) + '_' + \
                        str(int(100*te_neg_acc)) + '_' + \
                        str(loss).zfill(4) + '_' + \
                        str(val_loss).zfill(4) + '_' + \
                    '_reb_' + \
                        str(round_ct) + '_' + \
                        str(epochs) + '_' + \
                        str(batch_size) + '_' + \
