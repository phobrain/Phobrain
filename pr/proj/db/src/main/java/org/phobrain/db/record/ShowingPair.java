...
public class ShowingPair {

    private static final Logger log = LoggerFactory.getLogger(ShowingPair.class);
    ...
    public void analyzeDots() {
        ...
        if (dotHistory == null) {
            log.info("ShowingPair.analyzeDots: no dotHistory");
            return;
        }
        if (dotStartScreen == 0) {
            log.info("ShowingPair.analyzeDots: startScreen==0");
            return;
        }
