...
# === ppred.py - multithreaded batch pair predictions
...
if len(doneList) > 0:
    modelList = [x for x in modelList if x not in doneList]
...
print('-- model max/avg: ' + str(max_sz) + '/' + str(tot_sz/len(modelList)))
...
if me == 0:
    print('-- handing out models: ' + str(len(modelList)))

for i in range(0, len(modelList) ):
    if i % nproc == me:
        myModList.append(modelList[i])  # nproc1/evens

print('-- proc' + str(me) + '/' + str(os.getpid()) + ': ' + str(myModList))
